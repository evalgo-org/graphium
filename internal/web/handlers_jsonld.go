...
// JSONLDDeploymentsList shows all JSON-LD deployments
func (h *Handler) JSONLDDeploymentsList(c echo.Context) error {
	var user *models.User
	if claims, ok := c.Get("claims").(*auth.Claims); ok {
		user, _ = h.storage.GetUser(claims.UserID)
	}
	...
	listURL := fmt.Sprintf("http://localhost:%d/api/v1/stacks/jsonld/deployments", h.config.Server.Port)

	req, err := http.NewRequest("GET", listURL, nil)
	if err != nil {
		return Render(c, JSONLDDeploymentsListPage([]deploymentStateResponse{}, user))
	}
	...
	body, _ := io.ReadAll(resp.Body)
	var deployments []deploymentStateResponse
	if err := json.Unmarshal(body, &deployments); err != nil {
		return Render(c, JSONLDDeploymentsListPage([]deploymentStateResponse{}, user))
	}

	return Render(c, JSONLDDeploymentsListPage(deployments, user))
}

// JSONLDDeploymentsTable renders just the deployments table (for HTMX)
func (h *Handler) JSONLDDeploymentsTable(c echo.Context) error {
	// Call the API list endpoint
	listURL := fmt.Sprintf("http://localhost:%d/api/v1/stacks/jsonld/deployments", h.config.Server.Port)

	req, err := http.NewRequest("GET", listURL, nil)
	if err != nil {
		return c.String(http.StatusInternalServerError, "Failed to fetch deployments")
	}
	...
	var deployments []deploymentStateResponse
	if err := json.Unmarshal(body, &deployments); err != nil {
		return c.String(http.StatusInternalServerError, "Failed to parse deployments")
	}
	...
	pagination := PaginationInfo{
		Page:       1,
		PageSize:   100,
		TotalItems: len(deployments),
		TotalPages: 1,
		HasPrev:    false,
		HasNext:    false,
	}

	return Render(c, JSONLDDeploymentsTableWithPagination(deployments, pagination, ""))
}
